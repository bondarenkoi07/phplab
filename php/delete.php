        ...
        if($_SESSION['rigths']=='admin')
            foreach ($_GET as $key => $value) {
                if (isset($_GET[$key]) && $value == 'delete') {
                    ...
                    $query->execute();
                    if ($query->execute())
                        header("refresh:0;url='http://localhost:8080/php/show.php?delete=ok' ");
                    else
                        header("refresh:0;url='http://localhost:8080/php/show.php?delete=wrong' ");
                    return 0;
                }
            }
        header("refresh:0;url='http://localhost:8080/php/show.php?delete=empty' ");
        return 1;
    }
}
